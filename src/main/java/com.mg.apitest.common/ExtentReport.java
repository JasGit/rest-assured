...
/**
 * Created by jaspreetwalia on 10/10/2016.
 */
public class ExtentReport implements IReporter, IMethodInterceptor{
    ExtentReports extent;


    public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
        extent = new ExtentReports(outputDirectory + File.separator + "Extent.html", NetworkMode.ONLINE);
    ...
    private void buildTestNodes(IResultMap tests, LogStatus status) {
        ...
        if (tests.size() > 0) {
            ...
            for (ITestResult result : tests.getAllResults()) {
                ...
                test = extent.startTest(result.getMethod().getMethodName()+ newLine +"-----"+ result.getMethod().getConstructorOrMethod().getMethod().getAnnotation(Test.class)
                        .description());
